...
The system SHALL resolve an authenticated external identity by the combination of issuer and subject. Email and other mutable claims SHALL NOT be used as the stable identity key.
...
- **WHEN** a valid external identity is linked to a suspended, blocked, or otherwise inactive Customer
...
- **WHEN** an authenticated browser submits a state-changing request without its valid CSRF token
