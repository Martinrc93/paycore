...
- [x] 8.2 Run the full `mvnw test` suite with Docker available and resolve all regressions (2026-08-09 final run: `./mvnw.cmd test`; 193 tests, 0 failures, 0 errors, 0 skipped; durable evidence in `docs/verification/customer-authentication-task-8.md`).
...
- [x] 8.4 Perform a security-focused code review for credential/token leakage, session fixation, CSRF bypass, cookie scope, privilege boundaries, and failure-message disclosure (after the initial review, two remediation rounds, and independent fix re-review 1, the final whole-branch review found two Important and one Medium issue; the final fix wave resolved all three under strict RED/GREEN evidence; an independent scoped re-review approved all fixes with no new Critical, Important, or Medium breakage; durable chronology in `docs/reviews/customer-authentication-task-8.md` and `docs/verification/customer-authentication-task-8.md`).
