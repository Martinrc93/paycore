...
- [x] 8.3 Review the implementation against every `identity/customer-authentication` scenario, the architecture ADRs, UTC requirements, Flyway rules, and the OpenSpec task list (all 21 scenarios, ADR-0001/0002/0004, V1 immutability, V2 ordering, and tasks 1.1-8.4 evidenced in `docs/verification/customer-authentication-task-8.md`).
- [x] 8.4 Perform a security-focused code review for credential/token leakage, session fixation, CSRF bypass, cookie scope, privilege boundaries, and failure-message disclosure (initial independent review followed premature closure and found four Important issues; Fix Round 1 addressed build/warning/scanner issues but closed 8.4 before independent re-review; independent fix re-review 1 then confirmed those three fixes while leaving chronology and durable evidence open; Fix Round 2 closes 8.4 only after that re-review existed by adding tracked evidence under `docs/verification/customer-authentication-task-8.md`, `docs/reviews/customer-authentication-task-8.md`, and `scripts/scan-customer-authentication-secrets.ps1`; this round has not yet been independently re-reviewed).
